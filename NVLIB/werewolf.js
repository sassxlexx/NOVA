...
async function pagi(szhere, x, data) {
    ...
    return await szhere.sendMessage(x.room, {
        text: pagii(x),
        contextInfo: {
            externalAdReply: {
...
async function voting(szhere, x, data) {
    let row = [];
    let ment = [];
...
async function malam(szhere, x, data) {
...
async function skill(szhere, x, data) {
    skillOn(x.room, data)
        ...
        for (let i = 0; i < x.player.length; i++) {
            if (x.player[i].role === "werewolf") {
                if (x.player[i].isdead != true) {
                    ...
                    await szhere.sendMessage(x.player[i].id, {
                        text: textnya,
                        mentions: membernya,
                    });
                }
...
async function win(x, t, szhere, data) {
    ...
    if (getWinner(x.room, data).status === false || x.iswin === false) {
        ...
        return await szhere.sendMessage(sesinya, {
            text: textnya,
            contextInfo: {
                externalAdReply: {
                    title: "W E R E W O L F",
...
// playing
async function run(szhere, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, szhere, data);
            break;
        } else {
            await sleep(90000);
        }
        if (getWinner(id, data).status != null) {
            ...
            await pagi(szhere, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, szhere, data);
            break;
        ...
        if (getWinner(id, data).status != null) {
            ...
            await voting(szhere, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, szhere, data);
            break;
        ...
        if (getWinner(id, data).status != null) {
            ...
            await malam(szhere, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, szhere, data);
            break;
        } else {
            await skill(szhere, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) break;
    }
    await win(sesi(id, data), 1, szhere, data);
}

async function run_vote(szhere, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, szhere, data);
            break;
...
async function run_malam(szhere, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, szhere, data);
            break;
...
async function run_pagi(szhere, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, szhere, data);
            break;
